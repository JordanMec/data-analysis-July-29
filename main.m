...
dataDir = 'C:\Users\jorda\MATLAB Drive\Digital Twin - SAPPHIRES\RESULTS digital twin\20 96 hour results';
if isempty(dataDir)
...
for i = 1:length(configs)
    ...
    fprintf('  %s: PM2.5=%.3f (±%.3f), PM10=%.3f (±%.3f)\n', ...
        strrep(config, '_', '-'), ...
        ioData.stats.pm25_mean, ...
        abs(diff(ioData.stats.pm25_range))/2, ...
        ioData.stats.pm10_mean, ...
        abs(diff(ioData.stats.pm10_range))/2);
end
    ...
    for i = 1:length(triggerConfigs)
        ...
        if isfield(activeAnalysisResults.triggerResponse.(config), 'metrics')
            ...
            if isfield(metrics, 'avg_response_time') && ~isnan(metrics.avg_response_time)
                fprintf('  %s: Response Time=%.1fh, Active=%.0f%% of time\n', ...
                    strrep(config, '_', '-'), ...
...
if isfield(activeAnalysisResults, 'filterComparison')
    ...
    for i = 1:length(locations)
        ...
        if isfield(comparison, 'hepa') && isfield(comparison, 'merv')
            fprintf('  %s:\n', location);
            fprintf('    HEPA: I/O PM2.5=%.3f, Peak Reduction=%.1f%%\n', ...
                comparison.hepa.avg_io_ratio_pm25, ...
                comparison.hepa.peak_reduction);
            fprintf('    MERV: I/O PM2.5=%.3f, Peak Reduction=%.1f%%\n', ...
                comparison.merv.avg_io_ratio_pm25, ...
                comparison.merv.peak_reduction);
        end
    end
end
